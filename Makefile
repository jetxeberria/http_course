...
export CURRENT_VERSION:=$(shell python3.6 -c "import os; about={}; exec(open(os.path.join('http_course', '_meta.py')).read(), about); print(about['__version__'])")
...
uninstall: ## uninstall the package
	pip uninstall -y $(PIP_ARGS) http_course || true
...
env-upgrade-package: ## upgrade the package specified on PACKAGE
	@[ "${PACKAGE}" ] || ( echo "PACKAGE is not set"; exit 1 )
	pip-compile --upgrade-package ${PACKAGE} --no-emit-index-url --no-header --no-emit-trusted-host --output-file requirements.txt requirements.in

env-upgrade-dev-package: ## upgrade the development package specified on PACKAGE
	@[ "${PACKAGE}" ] || ( echo "PACKAGE is not set"; exit 1 )
	pip-compile --upgrade-package ${PACKAGE} --no-emit-index-url --no-header --no-emit-trusted-host --output-file requirements-dev.txt requirements-dev.in

env-upgrade-all: ## upgrade all dependencies
	pip-compile --upgrade --no-emit-index-url --no-header --no-emit-trusted-host --output-file requirements.txt requirements.in
	pip-compile --upgrade --no-emit-index-url --no-header --no-emit-trusted-host --output-file requirements-dev.txt requirements-dev.in

### QUALITY

lint: ## static code analysis with pylint
	pylint --rcfile http_course/.pylintrc --disable=$(EXTRA_DISABLE) -j 0 http_course
...
coverage: ## check test coverage
	coverage run --source http_course -m pytest
...
security: ## check source code for vulnerabilities or dangerous code
	bandit -c .bandit.yml -v -r http_course

security-report: ## check source code for vulnerabilities and generate an HTML report
	@mkdir -p docs/_build/security
	bandit -c .bandit.yml -v -r -f html -o docs/_build/security/index.html http_course
...
docs: ## generate project docs
	rm -f docs/http_course.rst
	find docs -name 'http_course.*.rst' -exec rm -f {} +
	rm -f docs/modules.rst
	sphinx-apidoc -o docs/ http_course
	$(MAKE) -C docs clean
	$(MAKE) -C docs html
...
doc-test-plan: dir-health
	...
	@sed -i -e 's/project_name/http_course/g' docs/health/test_plan.rst
...
doc-release: dir-operations
	...
	@sed -i -e 's/project_name/http_course/g' docs/operations/release.rst
	@echo Release created!

doc-run: dir-operations
	@cp -n ~/projects/daskeleton/templates/operations/run.rst docs/operations/run.rst
	@sed -i -e 's/project_name/http_course/g' docs/operations/run.rst
	@echo Run created!
...
doc-usage: dir-support
	...
	@sed -i -e 's/project_name/http_course/g' docs/support/usage.rst
	@sed -i -e 's/project_name/http_course/g' docs/support/usage.rst
	@echo Usage created!
...
publish-dev: ## publish packages to the repository.
	...
	twine upload --repository dasnano dist/http_course*
...
docker-run: ## run the specified command on docker (defaults to /bin/bash)
	docker-compose -f docker/dev/docker-compose.yml run --rm -u $(CURRENT_USER) http_course $(MAKE) $(TARGET)

docker-shell: ## drop the user into a shell inside a docker container
	docker-compose -f docker/dev/docker-compose.yml run --rm -u $(CURRENT_USER) http_course
...
ci-dist-dev: ## build the package (for development) for every supported version on CI (GitLab)
	tox -e http_course -- $(MAKE) dist-dev

ci-publish: ## publish built packages to the appropriate repository on CI (GitLab)
	tox -e http_course -- $(MAKE) publish ## publish the packages generated with dist or dist-dev on CI (GitLab)
...
clean-build: ## remove build artifacts
	...
	find http_course -name '*.c' -exec rm -f {} +
